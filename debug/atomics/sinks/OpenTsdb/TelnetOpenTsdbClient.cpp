...
TelnetOpenTsdbClient::TelnetOpenTsdbClient(const std::string & host, unsigned short port)
		  ...
		  maxQueued(0)
    {
		...
		printLog(1, "[TelnetOpenTSDBClient] start_connect \n");
        start_connect();
    }
...
{
	printLog(1, "[TelnetOpenTSDBClient] Running thread\n");
	...
	const TagsType * const ptags)
{
	if(this->closeRequested.load()){
		printLog(1, "[TelnetOpenTSDBClient] warning: attempted to add_point after closing the connection. Skipping point: %s %f %f (modelName=%s)\n", metric.data(), timestamp, value, ptags->at("modelName").data());
		return;
	}

	if(this->isClosed()){
		printLog(1, "[TelnetOpenTSDBClient] warning: attempted to add_point when the connection is closed. Skipping point: %s %f %f (modelName=%s)\n", metric.data(), timestamp, value, ptags->at("modelName").data());
	...
	if(this->pendingWrites.load() >= 1000){
		...
		printLog(1, "[TelnetOpenTSDBClient] Big load detected. Sleeping for 1 second. (PendingWrites=%u ; Successful writes: %u) \n", this->pendingWrites.load(), this->writeSuccess.load());

		//boost::this_thread::sleep(boost::posix_time::milliseconds(500));
		sleep(1); // TODO: one second might be too much... problems compiling using boost::thread :-(

		printLog(1, "[TelnetOpenTSDBClient] Waking up. (PendingWrites=%u; Successful writes: %u) \n", this->pendingWrites.load(), this->writeSuccess.load());
//		std::string version = this->getVersion(boost::posix_time::seconds(5));
//		if(version.find("2.1") == std::string::npos){
//			printLog(1, "[TelnetOpenTSDBClient] WARNING: Wrong OpenTsdb version. Expecting '2.1' but server returned: %s \n", version.data());
...
{
//		printLog(1, "[TelnetOpenTSDBClient] writting point: %s. Pending Writes: %u\n", addPointCommand.data(), this->pendingWrites.load());
...
//		printLog(1, "[TelnetOpenTSDBClient] Point written: %s. Pending Writes: %i\n", addPointCommand.data(), this->pendingWrites.load());
}
	...
	const TagsType * const ptags)
{
	...
	if (timestamp >= 4294967296){
			printLog(1, "[TelnetOpenTSDBClient] ERROR: Max timestamp exceeded. Trying to log %lu and the maximum timestamp allowed by OpenTsdb is 4294967296. \n", timestamp);
		...
		if(formatWarningMessages++ % 1000 == 0) printLog(1, "[TelnetOpenTSDBClient] WARNING: Big timestamp detected %lu (>2147483648), which required 64bit representation. Some applications/OpenTsdb interfaces might not support it.. \n", timestamp);
	}
...
void TelnetOpenTsdbClient::start_connect()
{
	...
	try{
		...
		printLog(1, "[TelnetOpenTSDBClient] ERROR attempting to connect to %s:%i. Check that OpenTsdb server is up and running in the web browser: http://%s:%i \n %s", this->m_host.data(), this->m_port, this->m_host.data(), this->m_port, e.what());
		throw e;
	}
}
...
void TelnetOpenTsdbClient::handle_write_error(std::string error){
	this->writeErrors++;
	printLog(1, "[TelnetOpenTsdbClient] %s",error.data() );
	//this->do_close();
...
std::string TelnetOpenTsdbClient::getVersion(boost::posix_time::time_duration timeout){
	...
	if (ec) {
		printLog(1, "[TelnetOpenTSDBClient] reading version TIMEOUT. Check OpentTSDB server is responsive. Can you issue a 'version' command from telnet %s %i ?  \n", this->m_host.data(), this->m_port);
		this->forceClose();
...
void TelnetOpenTsdbClient::check_deadline()
{
	...
	//printLog(1, "[TelnetOpenTSDBClient] Checking timer \n");
	if (this->m_reconnect_timer.expires_at() <= boost::asio::deadline_timer::traits_type::now())
	{
		printLog(1, "[TelnetOpenTSDBClient] Timer Expired, closing connections \n");
