...
//void printLog(const char *fmt, const char* fileName, ...);
void vprintLog(const char *fmt, va_list va);
//void vprintLog(const char *fmt, const char* fileName, va_list va);
...
static std::map<std::string, double> VarCache;

extern std::vector<std::string> CmdLineModelParams;
...
void putScilabVar(const char *varname,double v);
