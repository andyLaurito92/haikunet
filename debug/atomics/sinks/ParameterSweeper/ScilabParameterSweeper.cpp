...
int ScilabParameterSweeper::getCurrentExperimentNumber(){
	...
	try {
		...
		printLog(LOG_LEVEL_IMPORTANT, "[INIT] ScilabParameterSweeper: (WARNING) Scilab variable %s was not declared. Variable forced to 0  \n", this->simuRunVariableName.data());
...
int ScilabParameterSweeper::getTotalNumberOfExperiments(){
	...
	printLog(LOG_LEVEL_DEBUG, "Params done \n");
...
void ScilabParameterSweeper::UpdateParameters(){
	...
	char* parameterNames[30];
	//parameterNames[0] = "l1EventRate";
	//parameterNames[0] = "nRequestCredits";
	//parameterNames[0] = "l2Requests";
	//parameterNames[0] = "nDCM";
	parameterNames[0] = "flow2.periodDistribution_mu";

	// Second parameter
	parameterNames[1] = "flow2.periodDistribution_var";

	// Third Parameter
	parameterNames[2] = "lambda";
...
void ScilabParameterSweeper::onExit(){
	...
	//	sprintf(command,"%s = 0;", this->simuRunVariableName.data()); // TODO: doint this makes last Experiment to be numbered as 0 in the results.
	//}
