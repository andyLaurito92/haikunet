...
#ifndef ATOMICS_VECTOR_IVECTORIALMODEL_H_
#define ATOMICS_VECTOR_IVECTORIALMODEL_H_
...
public:
	...
	void init(double t,...) {
		...
		D0 = new Coupling(this->getFullName().data());
		...
		for (int i = 0; i < scalarInstances; i++) {
			childi = new char[64];
			sprintf(childi, "%s_%i", this->getFullName().data(), i); // Using full name as name for scalars. This will create the impression that the scalar models are in the hierarchy of the main simulation, but they are really not. They are hierarchically disconnected from the main simulation
